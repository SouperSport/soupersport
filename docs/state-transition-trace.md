...
This document defines what is meant by a *state‑transition trace* in SouperSport.
...
The following do *not* count as state, provided they are fully determined by
...
- Advancing control flow (entering/exiting a branch or iteration).
...
Exclusion is permitted only when recomputation is uniquely determined by recorded
...
The deterministic trace begins where sealed inputs and declared initial state are fixed.

## Relationship to Counterfactual Execution

Counterfactual execution varies assumptions while holding the baseline trace fixed.
...
- replace debuggers/profilers,
- model machine/OS state.
